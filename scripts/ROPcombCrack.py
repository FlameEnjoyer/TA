...
# --- Modified for multiple cracking fractions ---
CRACK_FRACTIONS       = [0.28, 0.5, 0.75] # List of cracking fractions to simulate
UF                    = 1.0  # Single Fuel Velocity for all runs
UA                    = 1.0  # Single Air Velocity for all runs
...
FIG_DIR               = Path("results/hasil_NO_comparison")
...
def run_and_analyze_flame(f_crack, uf, ua, width):
    ...
    nox_species_to_analyze = ['NO']  # Only analyze NO for this example
...
def plot_combined_line_plots(all_results):
    ...
    f_cracks = [res['f_crack'] for res in all_results]
    ...
    linestyle_handles = [plt.Line2D([0], [0], color='black', linestyle=s) for s in linestyles[:len(f_cracks)]]
    linestyle_labels = [f'{f:.2f}' for f in f_cracks]
    ax_rop1.legend(handles=linestyle_handles, labels=linestyle_labels, loc='upper right', title='Cracking fraction')
    ...
    fig_rop.suptitle(f'Comparison of Net NOx ROP Profiles\n$u_f$={all_results[0]["uf"]:.2f} m/s, $u_a$={all_results[0]["ua"]:.2f} m/s', fontsize=12)
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    fig_path = FIG_DIR / "combined_net_rop_profiles_NO.png"
    ...
    mf_linestyle_handles = [plt.Line2D([0], [0], color='black', linestyle=s) for s in linestyles[:len(f_cracks)]]
    mf_linestyle_labels = [f'{f:.2f}' for f in f_cracks]
    ax_mf1.legend(handles=mf_linestyle_handles, labels=mf_linestyle_labels, loc='upper right', title='Cracking fraction')
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    fig_path = FIG_DIR / "combined_mf_profiles_NO.png"
...
def plot_grouped_rop_bar_chart(all_results, target_species=TARGET_SPECIES_ROP_BAR):
    ...
    reference_reactions = []
    # Combine top producing and consuming reactions from the reference case
    ...
    if not reference_reactions:
        ...
        return
    # Sort reactions by their contribution in the reference case for a logical order
    ...
    for result in all_results:
        ...
        if idx_max_rop != -1:
            ...
            case_contributions = [0] * len(ref_reaction_indices) # Append zeros if flame failed
        all_contributions.append(case_contributions)

    # 3. Plot the grouped bar chart
    plot_data = np.array(all_contributions).T # Shape: (n_reactions, n_cases)
    ...
    for i in range(n_cases):
        ...
        ax.barh(y + offset, plot_data[:, i], height=bar_width, label=f'f={f_cracks[i]:.2f}', color=colors[i])
    ...
    ax.invert_yaxis() # Puts largest contributor on top
    ax.axvline(0, color='black', linewidth=0.8)
    ax.xaxis.set_major_formatter(mticker.FormatStrFormatter('%.1e'))
    ref_z_mm = ref_result['grid'][info_ref['grid_idx_max']] * 1000
    ax.set_title(f'Comparison of ROP Contributions with Varying Cracking Fractions')
    ax.legend(title="Crack Fraction")
    ax.grid(axis='x', linestyle=':', which='both')
    plt.tight_layout()

    fig_path = FIG_DIR / f"grouped_bar_chart_{target_species}_rop.png"
...
def main():
    ...
    print(f"Cracking Fractions to run: {CRACK_FRACTIONS}")
    print(f"Constant Conditions: Fuel Velocity = {UF} m/s, Air Velocity = {UA} m/s, Width = {WIDTH*1000} mm")
    print("-" * 70)

    all_results = []
    for f_crack in CRACK_FRACTIONS:
        print(f"\nProcessing case: Cracking Fraction = {f_crack:.2f}")
