...
#
# Output:
# 1. An Excel file ('data/extinction_data.xlsx') containing the complete dataset
#    of results, ready for machine learning, as well as separate sheets
#    for each specific case.
# 2. A series of PNG plot files ('results/figures/*.png') showing the 2D
...
TEMP_THRESHOLD = 1400.0                  # Extinction threshold temperature [K]

# --- PARALLEL PROCESSING ---
# Use one less than the total number of CPU cores, or at least 1
CPUS = max(mp.cpu_count() - 10, 1)
...
uf_range        = np.arange(1.0, 3.0, 1.0)
# Range of air velocities [m/s]
ua_range        = np.arange(1.0, 3.0, 1.0)  
# Burner widths [meters]
widths          = [0.01]      # Corresponds to 1 cm, 2 cm, 3 cm

# --- OUTPUT CONFIGURATION ---
OUT_XLSX = Path("data/extinction_data_new.xlsx")
FIG_DIR  = Path("results/figures_baru")
...
def print_progress(count, total, f, uf, ua, w, Tmax, elapsed):
    ...
    status_line = (
        ...
        f"Last Case: [f={f:.2f}, uf={uf:.1f}, ua={ua:.1f}, w={w*1000:.0f}mm] -> T_max={Tmax:.0f}K | "
...
def main():
    ...
    for f_val in crack_fractions:
        for width_val in widths:
            ...
            plt.title(f"Extinction Map\nf = {f_val:.2f}, width = {width_val*1000:.0f} mm")
            plt.legend()
            plt.tight_layout()
