...
def run_single_flame_and_analyze_rop(f_crack, uf, ua, width):
    ...
    fig_rop_net, ax_rop_net1 = plt.subplots(figsize=(8, 6))
    ...
    fig_rop_net.suptitle(f'Net NOx ROP Profiles\nFuel: Cracked NH3 (f={f_crack}), $u_f$={uf:.2f} m/s, $u_a$={ua:.2f} m/s', fontsize=10)
    ...
    fig_path_net = FIG_DIR / f"net_rop_profiles_f{f_crack:.2f}_uf{uf:.2f}_ua{ua:.2f}_w{int(width*1000)}.png"
    ...
    for sp_name in nox_species_to_analyze:
        ...
        if info['grid_idx_max'] != -1:
            ...
            #print(f"    Occurs at z = {z_max_rop_cm:.3f} cm, T = {info['T_at_max']:.2f} K")
        ...
        if info['grid_idx_min'] != -1 and info['min_val'] < -1e-12 :
            ...
            #print(f"    Occurs at z = {z_min_rop_cm:.3f} cm, T = {info['T_at_min']:.2f} K")
    ...
    fig_mf, ax_mf1 = plt.subplots(figsize=(8, 6))
    for i, sp_name in enumerate(nox_species_to_analyze):
    ...
    plt.tight_layout(rect=[0, 0, 1, 0.96])
    fig_path_mf = FIG_DIR / f"mf_profiles_f{f_crack:.2f}_uf{uf:.2f}_ua{ua:.2f}_w{int(width*1000)}.png"
    ...
    if target_species_for_line_plot in species_indices:
        ...
        if not selected_rxns_for_line_plot_info:
            ...
            plt.title(f'Key Reaction ROP Contributions to {target_species_for_line_plot}\nFuel: Cracked NH3 (f={f_crack}), $u_f$={uf:.2f}, $u_a$={ua:.2f}')
            plt.grid(False)
            plt.legend(loc='best', fontsize='small')
            plt.gca().yaxis.set_major_formatter(mticker.FormatStrFormatter('%.1e')) 
            
            fig_path_line = FIG_DIR / f"individual_rop_lines_{target_species_for_line_plot}_f{f_crack:.2f}_uf{uf:.2f}_ua{ua:.2f}_w{int(width*1000)}.png"
    ...
    if target_species_for_bar_chart in species_indices:
        ...
        if not reactions_for_bar_chart:
            ...
            # Sort reactions by contribution for a more organized chart
            reactions_for_bar_chart.sort(key=lambda x: x['contribution'], reverse=True)
            ...
            plt.figure(figsize=(10, max(6, len(labels) * 0.7)))
            bars = plt.barh(labels, contributions, color=['limegreen' if c > 0 else 'salmon' for c in contributions])

            # ==================== START: CODE TO CENTER X-AXIS AT 0 ====================
            # 1. Check if the contributions list is not empty to avoid errors.
            if contributions:
                # 2. Find the maximum absolute value in the contributions.
                max_abs_contribution = max(abs(c) for c in contributions)

                # 3. Set the x-axis limits to be symmetric around 0.
                #    A 10% padding (multiplication by 1.1) is added for better visual spacing.
                plt.xlim(-max_abs_contribution * 1.1, max_abs_contribution * 1.1)
            # ===================== END: CODE TO CENTER X-AXIS AT 0 =====================

            plt.xlabel('Rate of Production (mol·cm⁻³·s⁻¹)')
            plt.ylabel('Reaction')
            z_val_mm = flame.grid[info["grid_idx_max"]]*1000 if info["grid_idx_max"]!=-1 else np.nan
            plt.title(f'Key Reaction Contributions to {target_species_for_bar_chart}\nAt z={z_val_mm:.2f} mm (Max ROP location, f={f_crack})')
            plt.grid(axis='x', linestyle=':', which='both')
            plt.axvline(0, color='black', linewidth=0.8)
            plt.gca().invert_yaxis() # Puts largest contributor on top
            plt.tight_layout()
            plt.gca().xaxis.set_major_formatter(mticker.FormatStrFormatter('%.1e'))

            fig_path_bar = FIG_DIR / f"rop_bar_chart_{target_species_for_bar_chart}_f{f_crack:.2f}_uf{uf:.2f}_ua{ua:.2f}_w{int(width*1000)}.png"
